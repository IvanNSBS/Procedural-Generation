...
double ValueNoise_2D(double x, double y, int numOctaves, double persistence) {
  double total = 0,
...
public:
    int oct;
    double pers;
    PerlinNoise(const int oc = 7, const double p = 0.5): oct(oc), pers(p){}
    
    double get_noise(int x, int y)
    {
        return ValueNoise_2D(x, y, oct, pers);
    }
};